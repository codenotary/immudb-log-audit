...
vault-log-audit is a simple service and cli tool to store json formatted log input and audit it later in immudb Vault. 
...
### Reading data
Reading data from immudb Vault is as easy as
...
```bash
./vault-log-audit tail docker psql-postgresql-1 --stdout --stderr --follow
```
...
```bash
./vault-log-audit create kv k8s --indexes auditID+stage,kind,stage,user.username
```

Tail k8s.log 

```bash
./vault-log-audit tail file k8s test/k8s/k8s.log
```

Read

```bash
./vault-log-audit read kv k8s
./vault-log-audit read kv k8s user.username=admin
./vault-log-audit read kv k8s stage=ResponseStarted
...
```bash
./vault-log-audit audit kv k8s d4652481-193e-42f6-9b78-f8651cab5dfeRequestReceived
```
Note: as primary key is auditID+stage, in audit their values need to be concatenated. 

## Storing unstructured logs in immudb
vault-log-audit provides "wrap" parser, which wraps any log line with autogenerated uid and timestamp. In example, given following syslog line:
...
```bash
./vault-log-audit create sql syslog --parser wrap
```

Tail syslog 

```bash
./vault-log-audit tail file syslog test/syslog/syslog
```

Read

```bash
 ./vault-log-audit read sql syslog
 ./vault-log-audit read sql syslog "log_timestamp > CAST('2023-03-16 9:36:58.49' as TIMESTAMP)"
```

Audit

```bash
./vault-log-audit audit sql syslog
```
